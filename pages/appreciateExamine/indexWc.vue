<template>
	<view class="box">
		<view class="search">
			...
			<u--input style="background-color: white" placeholder="请输入户型编号" v-model="formData.hxbh"></u--input>
			<u-icon name="search" color="#909399" @click="ss" size="24"></u-icon>
		</view>
		<view class="result" v-if="show2">
			<view v-for="item in publicer.result" class="cretyuip">
				<u-swipe-action>
					<u-swipe-action-item :disabled="item.mainTable.tjsh=='通过'" :options="options2" @click="(value)=>itemclick(value,item)">
		...
		methods: {
			...
			async deldata(id) {
				let data = {
					...
					"header": {
						...
						"Md5": md5("Updatejpk" + 123 + this.setTime())
					}
				}
			...
			oncz() {
				this.classificationArry = [{
						...
						state: false,
						activeItem: [],
						option: JSON.parse(JSON.stringify(jplocation))
					},
					{
						name: '层数',
						state: false,
			...
			ontoUrls(itmer,detail1) {
				...
				uni.navigateTo({
					url: '/pages/hoseType/competitivehouseBJ?id=' + itmer.id
				})
			},
			clickItem(index) {
				if (index !== 'special') {
