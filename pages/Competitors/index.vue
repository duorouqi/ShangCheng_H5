	...
	<view class="box">
		<u--form :model="form" ref="uForm" labelWidth='auto' :rules="rules">
			...
			<view class="inputor">
				<u-form-item label="项目所在城市:" prop="xmszcs">
					<view class="input-hxbh input-type">
						<u-input fontSize='12px' placeholder="请输入" v-model="form.xmszcs" ></u-input>
					</view>
				</u-form-item>
			</view>
			<view class="inputor">
				<u-form-item label="项目具体地点:" prop="xmdd">
					<view class="input-hxbh input-type">
						<u-input fontSize='12px' placeholder="请输入" v-model="form.xmdd" ></u-input>
					</view>
				</u-form-item>
			</view>
			...
			<view class="inputor">
				<u-form-item label="套型:" prop="taoxing">
					<view class="input-hxbh input-type">
						<u-checkbox-group v-model="form.taoxing" style="display: flex;flex-wrap: wrap;">
		...
		<!-- 	<view class="box-information">
				<view class="box-information-one">
					...
					<u-form-item class="bzcmj" prop="bzcts" label="标准层套数:" borderBottom @click="showlx = true;actions=bzctslist">
						<view class="input-type">
					...
					<u-form-item class="sfdtrh" prop="sfdtrh" label="是否独梯入户:" borderBottom @click="showlx = true;actions=sfdtrhlist">
						<view class="input-type">
				...
				<view> -->
					...
					<u-form-item class='kfpj' prop="kfpj" label="可否拼接:" borderBottom @click="showlx = true;actions=kfpjlist;">
						<view class="input-type">
					...
					<u-form-item class='kfpj' prop="xmzt" label="项目状态:" borderBottom @click="showlx = true;actions=xmztlist;">
						<view class="input-type">
		...
		<!-- 	<view class="box-information-images-col">
			...
			<view class="inputor">
				<u-form-item prop="tjr" label="推荐人联系方式:">
					<view class="input-hxbh input-type">
						<u-input fontSize='12px' placeholder="请输入" style="width: 300rpx;" v-model="form.tjrlxfs" disabled>
						</u-input>
					</view>
				</u-form-item>
			</view>
	...
	export default {
		...
		methods: {
			...
			async postAddbzks() {
				...
				if (this.form.taoxing) {
					if (this.form.taoxing.length == 1) {
						taoxing = this.form.taoxing[0]
					} else {
						this.form.taoxing.forEach((ac, index) => {
							if (this.form.taoxing.length - 1 === index) {
								taoxing = taoxing + ',' + ac
							} else {
								if (index === 0) {
									taoxing = ac
				...
				try {
					let data = {
						...
						"header": {
							"systemid": "Updatejpknew",
							"currentDateTime": this.setTime(),
							"Md5": md5("Updatejpknew" + 123 + this.setTime())
						}
				}
				// console.log(JSON.stringify(data))
					let res = await postUpdatejpk(JSON.stringify(data));
					if (res.status == 1) {
						uni.navigateBack({
							delta: 1
						})
			...
			async getProjectlist() {
				...
				// this.form.xmdd = this.details.xmdd
				if(sessionStorage.getItem("detailsjp")){
					...
					for (let item in this.details) {
						...
						if(item=='xmdd'){
							this.form.xmdd = this.details[item]
						}
						// 定位
						if (item == 'dw') {
							let arry = []
							for (let i = 0; i < this.locations.length; i++) {
								this.details.dw.split(',').forEach(item => {
									if (item.indexOf(this.locations[i].name) !== -1) {
										arry.push(this.locations[i].id)
									}
								})
							}
							this.form.dw = arry
						...
						if (item == 'taoxing') {
							let arry = []
							let tx=''
							for (let i = 0; i < this.jpjacketeds.length; i++) {
								this.details.taoxing.split(',').forEach(item => {
									switch(item){
											case '二室二厅一卫（2F2T1W）':
												tx = '2室2厅1卫'
											break;
									...
									if (tx.indexOf(this.jpjacketeds[i].name) != -1) {
										arry.push(this.jpjacketeds[i].id)
									}
								})
							}
							this.form.taoxing = arry
						}
					// 	if (item == 'szxm') {
					// 		let arry = []
